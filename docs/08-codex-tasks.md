...
2. 数据库模型
3. 用户 / 权限 / 数据隔离
4. 安排、待办、计划、冲突、提醒等业务服务
5. Tool Executor
6. SchedulePlanningGraph
7. Agent Debug API
8. 通过 Debug API 验证 Agent 能识别安排并完成相关操作
9. Web Dashboard
10. 最后接入微信通道服务
```
...
结果：创建 task_item，生成安排草案，保存 pending_state
...
2. 创建安排时可创建 reminder_job。
3. 删除安排时取消 reminder_job。
4. 创建任务时保存 estimated_minutes、deadline、priority。
5. 生成安排草案时写入 `day_plans.status = draft`。
...
实现 LangGraph Agent 第一版，支持创建安排和查询安排。
...
2. 创建安排必须通过 ToolExecutor。
3. 查询安排必须通过 ToolExecutor。
...
```text
通过 Debug API 输入“明天下午 3 点开会”，能创建安排。
输入“明天有什么安排？”，能查询安排。
```
...
5. 用户回复序号后继续修改或删除。
6. 修改安排后更新 reminder_job。
7. 删除安排后取消 reminder_job。

### 验收标准

```text
输入“把明天下午 3 点的会议改到 4 点”可以修改安排。
输入“取消明天下午 4 点的会议”可以删除安排。
多个候选时可以通过序号选择。
```

## 16. Task 14：Agent 安全防护

### 目标

实现 Agent 5 层安全防御机制，防止 prompt 注入和数据泄露。

### 实现组件

```text
1. InputSanitizer - 输入消毒器
   - 检测 prompt 注入攻击
   - 限制输入长度（最大 2000 字符）
   - 关键词和正则模式匹配
...
3. ToolCallGuard - 工具调用守卫
   - 高风险工具需要用户确认
   - 验证 confirmed_action 参数

4. ToolResultSanitizer - 工具结果消毒器
   - 为用户数据添加安全标记
   - 防止工具结果被误解为系统指令

5. OutputSanitizer - 输出消毒器
   - 检测并过滤系统提示词泄露
   - 替换泄露内容为安全提示
```
...
```text
输入包含"忽略之前指令"的消息，系统返回安全提示。
输入超过 2000 字符的消息，系统返回长度限制提示。
高风险工具（如删除）需要用户确认才能执行。
工具结果包含安全标记，防止被误解为系统指令。
系统提示词不会泄露给用户。
```

## 17. Task 15：Agent 能力总体验收
...
## 18. Task 16：实现 Web Dashboard 基础页面
...
```text
Web 可以查看安排、待办、计划、冲突和 Agent 日志。
```

## 19. Task 17：实现 owner 管理页面
...
```text
curl 调用 /api/wechat/inbound，传入已绑定用户消息，能创建安排并返回 reply。
```
...
```text
先保证 SchedulePlanningGraph 能识别安排并完成相关操作；
再让微信消息进入这个已经稳定的 Agent。
```

## 23. 分批提交与部署验证规范

完成每个任务后，必须按照以下流程进行分批提交和部署验证：

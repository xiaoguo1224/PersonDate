...
所有 Service 必须添加 `logging` 日志，便于线上排查。日志级别约定：

### 业务操作日志

| 场景 | 级别 | 示例 |
|---|---|---|
| 创建成功 | `INFO` | `创建日程 user_id=xxx title=开会 item_id=abc123` |
| 更新成功 | `INFO` | `更新日程 user_id=xxx item_id=abc123 字段=title,start_time` |
| 删除成功 | `INFO` | `删除日程 user_id=xxx item_id=abc123` |
| 查询结果 | `DEBUG` | `查询日程 user_id=xxx date=2026-06-08 count=5` |
| 操作失败 | `WARNING` | `创建日程失败 user_id=xxx: 数据库错误` |
| 权限校验失败 | `WARNING` | `权限校验失败 user_id=xxx 尝试访问 item_id=abc123` |
| Agent 消息处理 | `INFO` | `Agent 消息 user_id=xxx intent=create_event` |
| 微信消息收发 | `INFO` | `微信消息入站 conversation_id=xxx content_type=text` |
| 提醒触发 | `INFO` | `提醒触发 job_id=xxx user_id=xxx` |
...
- 使用 `logger.info("操作描述 key1=%s key2=%s", val1, val2)` 格式（不要用 f-string）
- 每条日志必须包含 `user_id`（如有）用于按用户过滤
- 错误日志必须包含异常信息：`logger.warning("xxx 失败: %s", exc)`
...
## Task 2: 全量 Service 日志

在所有 Service 中添加结构化日志，便于线上排查。此 Task 在缓存接入之前完成，确保后续 Task 中的 Service 已有日志基础。

**Files:**
- Modify: `backend/app/services/auth_service.py`
- Modify: `backend/app/services/invite_code_service.py`
- Modify: `backend/app/services/user_service.py`
- Modify: `backend/app/services/channel_identity_service.py`
- Modify: `backend/app/services/agent_log_service.py`
- Modify: `backend/app/services/setup_service.py`
...
- [ ] **Step 2: InviteCodeService 添加日志**

修改 `backend/app/services/invite_code_service.py`：
...
class InviteCodeService:
    ...
    def validate(self, code: str) -> InviteCode:
        ...
        if invite_code is None:
            logger.warning("邀请码验证失败 code=%s: 无效", code)
            raise ValueError("邀请码无效")
        if invite_code.status != InviteCodeStatus.ACTIVE.value:
            logger.warning("邀请码验证失败 code=%s: 已失效 status=%s", code, invite_code.status)
        ...
        if invite_code.expires_at and invite_code.expires_at < datetime.now(UTC):
            ...
            logger.warning("邀请码验证失败 code=%s: 已过期", code)
            raise ValueError("邀请码已过期")
        if invite_code.used_count >= invite_code.max_uses:
            logger.warning("邀请码验证失败 code=%s: 已用完", code)
...
- [ ] **Step 3: UserService 添加日志**

修改 `backend/app/services/user_service.py`，在所有方法中添加日志：
...
- [ ] **Step 4: ChannelIdentityService 添加日志**

修改 `backend/app/services/channel_identity_service.py`：
...
- [ ] **Step 5: AgentLogService 添加日志**

修改 `backend/app/services/agent_log_service.py`：
...
- [ ] **Step 6: SetupService 添加日志**

修改 `backend/app/services/setup_service.py`：
...
- [ ] **Step 7: ScheduledItemService 添加日志**

修改 `backend/app/services/scheduled_item_service.py`，在 `create`、`update`、`soft_delete`、`mark_completed`、`confirm_drafts_for_date` 方法中添加日志：
...
在 `update_task` 方法中添加：
```python
        logger.info("更新任务 user_id=%s task_id=%s 字段=%s", task.user_id, task.id, ",".join(changes.keys()))
```

在 `complete_task` 方法中添加：
```python
...
- [ ] **Step 9: ReminderService 添加日志**

修改 `backend/app/services/reminder_service.py`，在 `create_for_target`、`create_from_scheduled_item`、`cancel_by_target`、`cancel_job`、`fire_due_jobs` 方法中添加日志：
...
在 `fire_due_jobs` 方法中添加：
```python
        logger.info("触发到期提醒 count=%d", len(jobs))
```

- [ ] **Step 10: ConflictService 添加日志**

修改 `backend/app/services/conflict_service.py`，在 `detect_item_conflicts`、`detect_day_conflicts`、`resolve_conflict`、`ignore_conflict` 方法中添加日志：
...
在 `update_settings` 方法末尾添加：
```python
        logger.info("更新系统设置 字段=%s", ",".join(payload.model_fields_set))
```

- [ ] **Step 12: PendingStateService 添加日志**

修改 `backend/app/services/pending_state_service.py`，在 `save`、`clear` 方法中添加日志：
...
- [ ] **Step 13: DailyNotificationService 添加日志增强**

修改 `backend/app/services/daily_notification_service.py`，在 `notify_user` 方法中添加关键日志：

```python
# 在 notify_user 方法开头添加
...
- [ ] **Step 14: 验证所有 Service 可导入**

Run: `cd backend && uv run python -c "from app.services.auth_service import AuthService; from app.services.invite_code_service import InviteCodeService; from app.services.user_service import UserService; from app.services.channel_identity_service import ChannelIdentityService; from app.services.agent_log_service import AgentLogService; from app.services.setup_service import SetupService; from app.services.scheduled_item_service import ScheduledItemService; from app.services.task_service import TaskService; from app.services.reminder_service import ReminderService; from app.services.conflict_service import ConflictService; from app.services.system_setting_service import SystemSettingService; from app.services.pending_state_service import PendingStateService; print('all imports ok')"`
Expected: `all imports ok`

- [ ] **Step 15: Commit**

```bash
git add backend/app/services/auth_service.py backend/app/services/invite_code_service.py backend/app/services/user_service.py backend/app/services/channel_identity_service.py backend/app/services/agent_log_service.py backend/app/services/setup_service.py backend/app/services/scheduled_item_service.py backend/app/services/task_service.py backend/app/services/reminder_service.py backend/app/services/conflict_service.py backend/app/services/system_setting_service.py backend/app/services/pending_state_service.py backend/app/services/daily_notification_service.py
git commit -m "feat(logging): 全量 Service 添加结构化日志"
```

---

## Task 3: CacheManager — 统一缓存读写接口
...
## Task 4: CacheInvalidator — 缓存失效逻辑
...
## Task 6: 后端缓存接入 — SystemSettingService
...
## Task 7: 后端缓存接入 — UserService (UserSettings)
...
## Task 8: 后端缓存接入 — ScheduledItemService
...
## Task 10: 后端缓存接入 — ReminderService
...
## Task 11: 后端缓存接入 — ConflictService + 查询优化
...
## Task 13: 后端缓存接入 — PendingStateService
...
## Task 14: Docker Compose 添加 Redis 服务
